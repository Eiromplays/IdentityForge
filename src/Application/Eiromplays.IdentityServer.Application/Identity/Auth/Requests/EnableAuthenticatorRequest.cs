...
{
    public EnableAuthenticatorRequestValidator(ICurrentUser currentUser, IStringLocalizer<EnableAuthenticatorRequestValidator> T)
    {
        RuleFor(u => u.Provider).Cascade(CascadeMode.Stop)
            ...
            .MustAsync(async (provider, __) =>
            {
                var userService = Resolve<IUserService>();
                return Enum.TryParse<TwoFactorAuthenticationProviders>(provider, true, out _) &&
        ...
        RuleFor(x => x.Code).NotEmpty().WithMessage("Verification code is required.").Length(6, 7)
            ...
            {
                var userService = Resolve<IUserService>();
                return (await userService.GetValidTwoFactorProvidersAsync(currentUser.GetUserId())).Contains(
