...
public class EnableAuthenticatorRequest
{
    ...
    public string Provider { get; set; } = TwoFactorAuthenticationProviders.App.ToString();
    public string Code { get; set; } = default!;
}

public class EnableAuthenticatorRequestValidator : Validator<EnableAuthenticatorRequest>
{
    public EnableAuthenticatorRequestValidator(IUserService _userService, ICurrentUser _currentUser, IStringLocalizer<EnableAuthenticatorRequestValidator> T)
    {
        RuleFor(u => u.Provider).Cascade(CascadeMode.Stop)
            ...
            .MustAsync(async (provider, __) => Enum.TryParse<TwoFactorAuthenticationProviders>(provider, true, out _) &&
                                           (await _userService.GetValidTwoFactorProvidersAsync(_currentUser.GetUserId()))
        ...
        RuleFor(x => x.Code).NotEmpty().WithMessage("Verification code is required.").Length(6, 7)
            ...
            .UnlessAsync(async (req, _) =>
                (await _userService.GetValidTwoFactorProvidersAsync(_currentUser.GetUserId())).Contains(
