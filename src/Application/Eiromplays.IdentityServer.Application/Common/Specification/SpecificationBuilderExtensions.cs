...
public static class SpecificationBuilderExtensions
{
        ...
        Search? search)
    {
        ...
        if (search.Fields.Any())
        {
            ...
            foreach (string field in search.Fields)
            {
                ...
                var propertyExpr = field.Split('.').Aggregate<string, Expression>(paramExpr, Expression.PropertyOrField);
        ...
        else
        {
            ...
            foreach (var property in typeof(T).GetProperties()
                         ...
                         propertyType && Type.GetTypeCode(propertyType) != TypeCode.Object))
            {
                var paramExpr = Expression.Parameter(typeof(T));
    ...
    {
        if (orderByFields is null) return new OrderedSpecificationBuilder<T>(specificationBuilder.Specification);

        foreach (var field in ParseOrderBy(orderByFields))
        {
            var paramExpr = Expression.Parameter(typeof(T));

            var propertyExpr = field.Key.Split('.').Aggregate<string, Expression>(
                paramExpr,
    ...
    private static Dictionary<string, OrderTypeEnum> ParseOrderBy(IEnumerable<string> orderByFields) =>
